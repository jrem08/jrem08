<h1 align="center">Hola 👋, Soy Juan </h1>
<h3 align="center">Ing de Software</h3>

---
Desarrollador de software Full Stack, cuento con experiencia en el desarrollo de aplicaciones web y móviles utilizando React, Node.js, Express, JavaScript y React Native. Cuento con sólidos conocimientos en MySQL, APIs REST, Postman y Git. Tengo interés en el rol de Soporte de Aplicaciones, donde aporto sólidos conocimientos en entornos Windows Server y Linux, utilizando PowerShell, Bash y Nginx. Me caracterizo por ser una persona proactiva, con gran capacidad de adaptación y aprendizaje continuo. 

* 🌍  Vivo en Pereira, Risaralda
* 🎓  Título obtenido [Técnico Profesional](https://www.acreditta.com/credential/d0308150-59a0-4751-b3b9-e12c065d2361?)
* 💻  Trabajé en  [ODEMAP Mosquera Sur](https://drive.google.com/drive/folders/1WWkDUuvJjELYUMXJ8zfJzWxo9fyXcRra?hl=es-419)
* 🚀  Actualmente estoy estudiando en [CIAF](https://www.ciaf.edu.co/software)
* 🧠  Estoy aprendiendo Pruebas de Software QA



----
<div align="left">
    <a href="https://skillicons.dev">
        <img src="https://skillicons.dev/icons?i=javascript,mysql,nodejs,react,git,linux,python&theme=dark" />
        <img src="https://skillicons.dev/icons?i=html,css,github,vscode,bootstrap,powershell,bash&theme=dark" />
        <img src="https://skillicons.dev/icons?i=postman,windows,materialui,express,npm,notion,tailwind&theme=dark" />
    </a>
</div>
</div>

---

